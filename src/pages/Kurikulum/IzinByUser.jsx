...
import TabelIzinByUser from "../../components/admin/Tabel/TableIzinByUser.jsx";
...
export default function IzinByUser() {
  const [Izin, setIzin] = useState([]);
  const [izinSearch, setIzinSearch] = useState([]);
  const { user } = useSelector((state) => state.user);
  const {iduser,role,tipe, statusFilter} = useParams();
  ...
  const fetchData = async () => {
    try {
      const result = await axios.get(
        `${BACKEND_BASE_URL}api/getIzinGuruById/${iduser}`
      );
      ...
      console.log("mang eak : ",result.data.results);
    } catch (err) {
      console.log(err);
    }
  };
  ...
  return (
    <div className="w-full min-h-screen flex">
      ...
      <div className={`w-11/12 mx-auto`}>
        <TopBar>Semua Izin {tipe == 'no' && statusFilter != 'no'? statusFilter : tipe == 'no' && statusFilter == 'no'? "":tipe}</TopBar>
        <div className="w-[95%] h-[10px] lg:justify-between justify-center xl:justify-between mx-auto flex">
          <div className=""></div>
        </div>

        {!addIzin ? <TabelIzinByUser data={izinSearch} role={role} tipe={tipe} statusFilter={statusFilter}/> : null}
      </div>
    </div>
  );
}
